...
#Region "Fields"
    ...
    Private _CreationUser As String = GlobalVariables.CurrUserForTasks
    Private _TaskStatus As Integer = 50
    Private _AssignedTo As Integer = 0
    Private _CreationDate2 As DateTime = Today
    Private _Referance2 As Integer = 0
    Private _UniqueID2 As Integer = 0
    Private _Service As Integer = 0
...
#End Region
    ...
    Public Overridable Sub LoadFormData(ByVal appointment As Appointment)
        'do nothing
        SearchServiceNo.Properties.DataSource = GetItems("Services")
        _CreationUser = appointment.CustomFields("CreationUser")
        _TaskStatus = appointment.CustomFields("TaskStatus")
        _AssignedTo = appointment.CustomFields("AssignedTo")
        _CreationDate2 = appointment.CustomFields("CreationDate")
        _Referance2 = appointment.CustomFields("Referance")
        _Service = appointment.CustomFields("ServiceNo")
        Me.UniqueID.EditValue = appointment.CustomFields("UniqueID")
        Me.SearchCreationUser.EditValue = _CreationUser
        Me.TextTaskStatus.EditValue = _TaskStatus
        Me.SearchAssignedTo.EditValue = _AssignedTo
        Me.CreationDate.DateTime = _CreationDate2
        Me.Referance.EditValue = _Referance2
        Me.BarTaskStatus.Caption = GetTaskName(_TaskStatus)
        Me.SearchServiceNo.EditValue = _Service

        ' Me.BarTaskStatus.Caption = GetTaskName(_TaskStatus)
        UniqueID.Visible = True

    End Sub
    Private Function GetTaskName(_StatusID As Integer) As String
        Dim _StatusName As String
        Dim Sql As New SQLControl
        Dim SqlString As String
        Try
            SqlString = " Select [StatusName] from [dbo].[CRMTaskStatuses]
                          where [StatusID]=" & _StatusID
            Sql.SqlTrueTimeRunQuery(SqlString)
            _StatusName = Sql.SQLDS.Tables(0).Rows(0).Item("StatusName")
    ...
    Public Overridable Function SaveFormData(ByVal appointment As Appointment) As Boolean
        appointment.CustomFields("CreationUser") = Me.SearchCreationUser.EditValue
        appointment.CustomFields("TaskStatus") = Me.TextTaskStatus.EditValue
        appointment.CustomFields("AssignedTo") = Me.SearchAssignedTo.EditValue
        appointment.CustomFields("CreationDate") = Me.CreationDate.DateTime
        appointment.CustomFields("Referance") = Me.Referance.EditValue
        appointment.CustomFields("ServiceNo") = Me.SearchServiceNo.EditValue
        Return True

    End Function
    Public Overridable Function IsAppointmentChanged(ByVal appointment As Appointment) As Boolean
        If _CreationUser = appointment.CustomFields("CreationUser") And _TaskStatus = appointment.CustomFields("TaskStatus") _
            And _AssignedTo = appointment.CustomFields("AssignedTo") And _CreationDate2 = appointment.CustomFields("CreationDate") _
            And _Referance2 = appointment.CustomFields("Referance") Then
            Return False
        Else
            Return True
        End If
    ...
    Private Sub Save(ByVal closeAfterSave As Boolean)
        ...
        If String.IsNullOrEmpty(tbSubject.Text) Then
            MsgBox("يجب كتابة ملاحظة")
            Return
        End If

        If IsNothing(SearchAssignedTo.EditValue) Then
            MsgBox("يجب اختيار موظف")
            Return
        End If
        ...
        If String.IsNullOrEmpty(UniqueID.EditValue) Then
            InsertToClosingTables(GetLastTaskID, "انشاء المهمة", Now, GlobalVariables.CurrUserForTasks, 0, 1)
        End If
        ...
        End If
    End Sub
    Private Function GetLastTaskID() As Integer
        Dim _TaskID As Integer
        Try
            Dim sql As New SQLControl
            Dim SqlString As String = " Select max(UniqueID) as TaskID from [Appointments] "
            sql.SqlTrueAccountingRunQuery(SqlString)
            _TaskID = sql.SQLDS.Tables(0).Rows(0).Item("TaskID")
        Catch ex As Exception
            _TaskID = 0
        End Try
        Return _TaskID
    ...
    Private Sub OutlookAppointmentForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'SearchCreationUser.EditValue
        Me.Text = "مهمة"
        Me.BarButtonDone.Visibility = BarItemVisibility.Never
        Me.BarButtonFollowing.Visibility = BarItemVisibility.Never
        Me.BarButtonClose.Visibility = BarItemVisibility.Never
        Me.btnDelete.Visibility = BarItemVisibility.Never
        Me.BarButtonDelete.Visibility = BarItemVisibility.Never

    End Sub

    Private Sub AppointmentBackstageControl_Load(sender As Object, e As EventArgs) Handles appointmentBackstageControl.Load
    ...
    Private Sub TextTaskStatus_EditValueChanged(sender As Object, e As EventArgs) Handles TextTaskStatus.EditValueChanged
        ...
        If Me.TextTaskStatus.EditValue = 3 Then
            btnSaveAndClose.Enabled = False
            BarButtonFollowing.Enabled = False
            BarButtonDone.Enabled = False
            BarTaskStatus.Caption = TextTaskStatus.Text
        End If
    ...
    Private Sub SearchAssignedTo_EditValueChanged(sender As Object, e As EventArgs) Handles SearchAssignedTo.EditValueChanged

    End Sub

    Private Sub Referance_EditValueChanged(sender As Object, e As EventArgs) Handles Referance.EditValueChanged
        tbLocation.Text = Referance.Text
    End Sub
    ...
    Private Sub edtStartDate_EditValueChanged(sender As Object, e As EventArgs) Handles edtStartDate.EditValueChanged
        Me.edtEndDate.DateTime = Me.edtStartDate.DateTime

    End Sub

    Private Sub edtStartTime_EditValueChanged(sender As Object, e As EventArgs) Handles edtStartTime.EditValueChanged
        'Me.edtEndDate.DateTime = Today
        'Me.edtEndTime.Time = Me.edtStartTime.Time.AddHours(1)

        'Try
        '    Controller.End = Controller.Start.AddHours(1)
        '    Controller.DisplayEndDate = Controller.DisplayStartDate
        'Catch ex As Exception
    ...
    Private Sub edtEndTime_EditValueChanged(sender As Object, e As EventArgs) Handles edtEndTime.EditValueChanged
        'Me.edtEndDate.DateTime = Me.edtStartDate.DateTime
        'Me.edtEndTime.Time.AddHours(1)
    End Sub
    Private Sub edtEndTime_Eleave(sender As Object, e As EventArgs) Handles edtEndTime.Leave
        'Me.edtEndDate.DateTime = Me.edtStartDate.DateTime
        'Me.edtEndTime.Time.AddHours(1)
    End Sub

    Private Sub BarButtonItem7_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BarButtonFollowing.ItemClick
        'If String.IsNullOrEmpty(UniqueID.Text) Then Return
        'Dim _SearchAssignedTo As Integer = SearchAssignedTo.EditValue
        'With My.Forms.AppointmentsClosing
        '    .TextTaskStatus.EditValue = 2
        '    .tbSubject.Text = Me.tbSubject.Text
        '    .CreationDate.DateTime = CreationDate.DateTime
        '    .UniqueID.EditValue = UniqueID.EditValue
        '    .edtStartDate.DateTime = edtStartDate.DateTime
        '    .SearchAssignedTo.EditValue = _SearchAssignedTo
        '    .Referance.EditValue = Referance.EditValue
        '    .TextTaskStatus.EditValue = TextTaskStatus.EditValue
        '    .tbDescription.EditValue = ""
        '    .SearchCreationUser.EditValue = SearchCreationUser.EditValue
        '    .TextTaskStatus.ReadOnly = True
        ...
        If String.IsNullOrEmpty(UniqueID.Text) Then Return
        Dim _SearchAssignedTo As Integer = SearchAssignedTo.EditValue
        With My.Forms.CRMFollowingTask
            .TextTaskStatus.EditValue = 2
            .tbSubject.Text = Me.tbSubject.Text
            ' .CreationDate.DateTime = CreationDate.DateTime
            .UniqueID.EditValue = UniqueID.EditValue
            ...
            ' .Referance.ReadOnly = True
            .CreationDate.DateTime = CreationDate.DateTime
            .BarStaticItem1.Caption = TextTaskStatus.Text
    ...
    Private Sub BarButtonItem4_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BarButtonDone.ItemClick

        CRMCloseTask.UniqueID.EditValue = Me.UniqueID.EditValue
        CRMCloseTask.SearchCreationUser.EditValue = Me.SearchCreationUser.EditValue
        CRMCloseTask.LastTaskStatus.EditValue = TextTaskStatus.EditValue
        CRMCloseTask.CurrentTaskStatus.EditValue = 3
        CRMCloseTask.MemoCloseNote.Select()
        Me.Dispose()
        CRMCloseTask.ShowDialog()


    End Sub

    Private Sub UniqueID_EditValueChanged(sender As Object, e As EventArgs) Handles UniqueID.EditValueChanged
        Me.Text = "مهمة"
        Me.BarButtonDone.Visibility = BarItemVisibility.Always
        Me.BarButtonFollowing.Visibility = BarItemVisibility.Always
        Me.BarButtonClose.Visibility = BarItemVisibility.Always
        Me.btnDelete.Visibility = BarItemVisibility.Always
        Me.BarButtonDelete.Visibility = BarItemVisibility.Always
        Me.btnDelete.Visibility = BarItemVisibility.Never
        If String.IsNullOrEmpty(UniqueID.Text) Then
            Me.BarButtonTaskTrans.Visibility = BarItemVisibility.Never
        End If


    End Sub

    Private Sub SearchCreationUser_EditValueChanged(sender As Object, e As EventArgs) Handles SearchCreationUser.EditValueChanged
        If _CreationUser <> GlobalVariables.CurrUserForTasks Then
            '   panel2.Enabled = False
            tbSubject.ReadOnly = True
            CreationDate.ReadOnly = True
            edtStartDate.ReadOnly = True
            SearchAssignedTo.ReadOnly = True
            Referance.ReadOnly = True
            tbDescription.Enabled = True
            'TextTaskStatus.ReadOnly = True
            btnSaveAndClose.Visibility = BarItemVisibility.Never
            BarButtonClose.Enabled = False
            rpgOptions.Visible = False
            btnDelete.Enabled = False
            BarButtonDelete.Visibility = BarItemVisibility.Never
    ...
    Private Sub BarButtonItem4_ItemClick_1(sender As Object, e As ItemClickEventArgs) Handles ButtonApplyLable.ItemClick
        OnOkButton()
    End Sub

    Private Sub BarButtonItem4_ItemClick_2(sender As Object, e As ItemClickEventArgs) Handles BarButtonTaskTrans.ItemClick
        If String.IsNullOrEmpty(UniqueID.Text) Then
            MsgBox("لا يوجد حركات")
        Else
            My.Forms.CRMTaskTrans.TextTaskID.EditValue = Me.UniqueID.EditValue
            My.Forms.CRMTaskTrans.DockPanel1.Close()
    ...
    Private Sub BarButtonClose_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BarButtonClose.ItemClick
        InsertToClosingTables(Me.UniqueID.EditValue, My.Forms.CRMCloseTask.MemoCloseNote.Text,
                      ...
                      TextTaskStatus.EditValue, 4)
        UpdateAppointment("Close", UniqueID.EditValue, tbDescription.Text,
                          Me.SearchCreationUser.EditValue, Now, Me.SearchCreationUser.EditValue)
        MsgBox("تم انجاز المهمة")
        Me.Dispose()
    End Sub

    Private Sub BarButtonItem4_ItemClick_3(sender As Object, e As ItemClickEventArgs) Handles BarButtonDelete.ItemClick
        If XtraMessageBox.Show("هل تريد حذف المهمة ؟", "Confirmation", MessageBoxButtons.YesNo) <> DialogResult.No Then
            Dim Sql As New SQLControl
            Dim SqlString As String
            SqlString = " Delete from [dbo].Appointments where UniqueID =" & Me.UniqueID.EditValue
            If Sql.SqlTrueAccountingRunQuery(SqlString) = "True" Then
                XtraMessageBox.Show("تم حذف المهمة")
                Me.Dispose()
    ...
    Private Sub Referance_Properties_AddNewValue(sender As Object, e As AddNewValueEventArgs) Handles Referance.Properties.AddNewValue
        ' If Main.FinancialPageCategory.Visible = True Then
    ...
    Private Sub SearchLookUpEdit1_Properties_BeforePopup(sender As Object, e As EventArgs) Handles SearchServiceNo.Properties.BeforePopup
