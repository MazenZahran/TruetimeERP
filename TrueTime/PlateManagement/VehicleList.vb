Public Class VehicleList
    ...
    Private Sub RefreshData()
        Try
            ...
            SqlString = " SELECT  [CarID]      ,[CARNO]      ,[ChassiNoCar]      ,[CarType] as MarkaCar     ,[CarModel] as ModelCar,
                              ...
                              [active]      ,[BegSpedometaer]      ,[CarDetails]      ,[Available]      ,[MaintenanceAccountNo]      ,[AssetsAccountNo],
                              ...
                              (SELECT top 1 EmployeeNo FROM [dbo].[VehiclesReceiptDeliveryEmloyees] V WHERE     V.CarID=C.[CarID]     ORDER BY [ID] desc) as DriverID,
                              (SELECT top 1 EmployeeName  FROM [dbo].[VehiclesReceiptDeliveryEmloyees] V left join EmployeesData E on V.EmployeeNo=E.EmployeeID  WHERE     V.CarID=C.[CarID]     ORDER BY V.[ID] desc) as DriverName
                   FROM [dbo].[Vehicles] C  "
            SqlString = SqlString + "  where C.CarID > 0   "
            Select Case CStr(RadioGroup2.EditValue)
                Case "1"
                    SqlString = SqlString + " and C.[active] = 1 "
                Case "2"
                    SqlString = SqlString + " and C.[active] = 0 "
            End Select
    ...
    Private Sub SimpleButton4_Click(sender As Object, e As EventArgs) Handles SimpleButton4.Click
        My.Forms.VehicleEdit.LabelFormName.Text = " اضافة مركبة جديدة "
        My.Forms.VehicleEdit.CheckActive.Checked = True
        My.Forms.VehicleEdit.TextSearchCarID.Text = -1
        My.Forms.VehicleEdit.TextCARNO.Select()
        If VehicleEdit.ShowDialog() = DialogResult.OK Then
            MsgBox("ok")
        Else
            RefreshData()
        End If
    ...
    Private Sub TileView1_DoubleClick(sender As Object, e As EventArgs) Handles TileView1.DoubleClick
        Dim CarID As String = CType(Me.TileView1.GetRowCellValue(TileView1.FocusedRowHandle, "CarID"), String)
        My.Forms.VehicleEdit.TextSearchCarID.Text = CarID
        If VehicleEdit.ShowDialog() = DialogResult.OK Then
            MsgBox("ok")
        Else
            RefreshData()
