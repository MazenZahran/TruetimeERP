...
Public Class AccountStatmentDetails
    ...
    Private thermalReport As AccountStatementReport80mm
        ...
        If _OpenForPrint = True Then
            Select Case _PrintOption
                ...
                Case "Print"
                    PrintReport("Print")
                Case "ThermalPrint"
                    GenerateThermalReport(Format(DateEditFrom.DateTime, "yyyy-MM-dd"), Format(DateEditTo.DateTime, "yyyy-MM-dd"), SearchReferance.EditValue, Currency.EditValue, False)
            End Select
            Me.Close()
        End If
    End Sub
    Public Sub ExpandAllRows(ByVal View As GridView)
        ...
        End Try
    End Sub
    Public Sub GetAcountStatmentForRef(_DateFrom As String, _DateTo As String, Referance As String, Currency As Integer)
...
DocName,DocNameValue,[DocManualNo],DocSort,DocNotes,Balance  from
(
                         select J.[DocCode] as [DocCode] ,J.[DocID] as [DocID],[DocDate],D.Name as DocName,DocName as DocNameValue,DocNotes"
...
from Journal J
                      ...
                      where DocName NOT IN (16, 17, 18, 19) And DocStatus > 0 and (DocDate between '" & Format(CDate(_DateFrom), "yyyy-MM-dd") & "' and '" & Format(CDate(_DateTo), "yyyy-MM-dd") & "')"
            SqlString += "  and (   DebitAcc = '" & RefData.RefAccID & "' OR CredAcc= '" & RefData.RefAccID & "') and  Referance = " & Referance
            SqlString += ") Z
group by [DocCode],[DocID],[DocDate],DocName,DocNameValue,[DocManualNo],DocSort,DocNotes,Balance order by DocDate,DocName,DocID  "
            ...
            If _ShowDetailsNote = True Then
                SqlString2 += " CONCAT(J.ItemName,'/',J.DocNoteByAccount) as ItemName ,"
            Else
                SqlString2 += " J.ItemName as ItemName , "
            End If

            SqlString2 += " StockID,StockUnit,StockQuantity, case when StockQuantity > 0 then CONCAT(Cast(DocAmount/Abs(StockQuantity) as DECIMAL(10,2) ) ,' ',case when StockUnit Is Null then '' else U.Code end ) else '0' End  as  StockPrice,StockPrice as PriceBeforeDiscount,UU.name as UnitName,
                               CONCAT(CAST(DocAmount AS DECIMAL(10,2))  ,' ',U.Code )  as DocAmount,U.Code as DocCurrency,[ExchangePrice],  BaseAmount,BaseCurrAmount,J.CheckNo,J.CheckDueDate,C.CostName"
            SqlString2 += "    From Journal J                 
                           left join Currency U on U.CurrID=J.DocCurrency
                           left join Units UU on UU.id=J.StockUnit
                           left join CostCenter C on C.CostID=J.DocCostCenter 
                      where DocName NOT IN (16, 17, 18, 19) And (StockID <> '0' or CheckNo <> 0) and DocStatus > 0 and (DocDate between '" & Format(CDate(_DateFrom), "yyyy-MM-dd") & "' and '" & Format(CDate(_DateTo), "yyyy-MM-dd") & "') "
            SqlString2 += "  and  Referance = " & Referance
            ...
            If CheckCalcBegBalance.Checked = True Then BegBalce = GetBegBalanceForRef(Referance, _DateFrom, _DateTo, Currency)
...
(select isnull(sum(BaseAmount),0) as CreditAmount from [Journal] Where DocStatus > 0 And DebitAcc ='0' And  [DocDate] < '" & Format(CDate(_DateFrom), "yyyy-MM-dd") & "'  " & _SearchType & "  ) as C"
    ...
    Private Sub PrintReport(_option As String)
        ...
        Else
            ...
            compositeLink.PrintingSystemBase.ShowMarginsWarning = False
    ...
    Private Sub TextEdit1_EditValueChanged(sender As Object, e As EventArgs) Handles TextEdit1.EditValueChanged
        GridView2.Columns("ItemName").FilterInfo = New ColumnFilterInfo("[ItemName] LIKE '%" & TextEdit1.Text & "%'")

        'GridView2.SetRowCellValue(GridControl.AutoFilterRowHandle, GridView2.Columns("ItemName"), TextEdit1.Text)
    ...
    Private Sub BarButtonItem4_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem4.ItemClick
        Select Case Me.ColPriceBeforeDiscount.Visible
            Case False
                Me.ColPriceBeforeDiscount.Visible = True
                Me.ColPriceBeforeDiscount.VisibleIndex = 3
            Case True
                Me.ColPriceBeforeDiscount.Visible = False
                '  Me.ColPriceBeforeDiscount.VisibleIndex = 3
        End Select
        GridView2.BestFitColumns()
    ...
    Private Sub BarButtonPrintPreview_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonPrintPreview.ItemClick
        PrintReport("Preview")
    End Sub
    ...
    Private Sub BarButtonItemPrintOnThermal_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItemPrintOnThermal.ItemClick
        GenerateThermalReport(Format(DateEditFrom.DateTime, "yyyy-MM-dd"), Format(DateEditTo.DateTime, "yyyy-MM-dd"), SearchReferance.EditValue, Currency.EditValue, True)
    End Sub
    Public Sub GenerateThermalReport(dateFrom As String, dateTo As String, reference As String, currency As Integer, Preview As Boolean)
        Try
            ' Get data using existing method
            Dim reportData As DataSet = GetThermalAccountStatementData(dateFrom, dateTo, reference, currency)

            If reportData IsNot Nothing Then
                ' Debug: Check data
                System.Diagnostics.Debug.WriteLine($"DataSet has {reportData.Tables.Count} tables")
                For Each table As DataTable In reportData.Tables
                    System.Diagnostics.Debug.WriteLine($"Table '{table.TableName}' has {table.Rows.Count} rows")
                Next

                ' Check if we have at least the Journal table with data
                If reportData.Tables.Contains("Journal") AndAlso reportData.Tables("JournalDetails").Rows.Count > 0 Then
                    ' Create and configure thermal report
                    thermalReport = New AccountStatementReport80mm()
                    Dim endingBalance As Decimal = 0
                    If reportData.Tables("Journal").Rows.Count > 0 Then
                        endingBalance = Convert.ToDecimal(reportData.Tables("Journal").Rows(reportData.Tables("Journal").Rows.Count - 1).Item("Balance"))
                    End If
                    Dim periodDates As String = $" من تاريخ: {dateFrom} إلى تاريخ: {dateTo}"
                    thermalReport.SetDataSource(reportData, GetRefranceData(CInt(reference)).RefName, endingBalance, periodDates)
                    ' Show print preview
                    Dim printTool As New DevExpress.XtraReports.UI.ReportPrintTool(thermalReport)
                ...
                Else
                    MessageBox.Show("لا توجد بيانات لعرضها في التقرير", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                MessageBox.Show("فشل في استرجاع البيانات", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If

        Catch ex As Exception
            MessageBox.Show($"خطأ في إنشاء التقرير: {ex.Message}", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error)
            System.Diagnostics.Debug.WriteLine($"Error in GenerateThermalReport: {ex.Message}")
        End Try
    End Sub
    Private Function GetThermalAccountStatementData(dateFrom As String, dateTo As String, reference As String, currency As Integer) As DataSet
        Try
            Dim SqlString As String = String.Empty
            Dim SqlString2 As String = String.Empty
            Dim JouranlTable As New DataTable
            Dim RefData = GetRefranceData(CInt(reference)) ' Convert reference to integer
            Dim AccData = GetFinancialAccountsData(RefData.RefAccID)
            Dim AccCurr = AccData.Currency

            SqlString = "Select case when sum(DebitAmount)=0 then -1 * sum(CredAmount) else sum(DebitAmount) end as Amount,sum(DebitAmount) As DebitAmount,sum(CredAmount) as CredAmount,[DocCode],[DocID],[DocDate],
DocName,DocNameValue,[DocManualNo],DocSort,DocNotes,Balance  from
(
                         select J.[DocCode] as [DocCode] ,J.[DocID] as [DocID],[DocDate],D.Name as DocName,DocName as DocNameValue,DocNotes"
            If currency = _DefaultCurr Then
...
from Journal J
                           ...
                           left join CostCenter C on C.CostID = J.DocCostCenter
                           left join Currency U on U.CurrID=J.DocCurrency
                      where DocName NOT IN (16, 17, 18, 19) And DocStatus > 0 and (DocDate between @DateFrom and @DateTo)"
            SqlString += "  and (   DebitAcc = @RefAccID OR CredAcc = @RefAccID) and  Referance = @Reference"
            SqlString += ") Z
group by [DocCode],[DocID],[DocDate],DocName,DocNameValue,[DocManualNo],DocSort,DocNotes,Balance order by DocDate,DocName,DocID  "
            ...
            If _ShowDetailsNote = True Then
                SqlString2 += " CONCAT(J.ItemName,'/',J.DocNoteByAccount) as ItemName ,"
            Else
                SqlString2 += " Concat(J.ItemName,' / ',UU.name) as ItemName , "
            End If

            SqlString2 += " StockID,StockUnit,StockQuantity, case when StockQuantity > 0 then CONCAT(Cast(DocAmount/Abs(StockQuantity) as DECIMAL(10,2) ) ,'',case when StockUnit Is Null then '' else '' end ) else '0' End  as  StockPrice,StockPrice as PriceBeforeDiscount,UU.name as UnitName,
                               CONCAT(CAST(DocAmount AS DECIMAL(10,2))  ,'' )  as DocAmount,U.Code as DocCurrency,[ExchangePrice],  BaseAmount,BaseCurrAmount,J.CheckNo,J.CheckDueDate,C.CostName"
            SqlString2 += "    From Journal J                 
                           left join Currency U on U.CurrID=J.DocCurrency
                           left join Units UU on UU.id=J.StockUnit
                           left join CostCenter C on C.CostID=J.DocCostCenter 
                      where DocName NOT IN (16, 17, 18, 19) And (StockID <> '0' or CheckNo <> 0) and DocStatus > 0 and (DocDate between @DateFrom and @DateTo) "
            SqlString2 += "  and  Referance = @Reference"
            SqlString2 += "  order by OrderID"

            Using Con As New SqlConnection(My.Settings.TrueTimeConnectionString)
                Con.Open()
                Using Adapter1 As New SqlDataAdapter(SqlString, Con)
                    Using Adapter2 As New SqlDataAdapter(SqlString2, Con)
                        ' Add parameters to prevent SQL injection
                        Adapter1.SelectCommand.Parameters.AddWithValue("@DateFrom", Format(CDate(dateFrom), "yyyy-MM-dd"))
                        Adapter1.SelectCommand.Parameters.AddWithValue("@DateTo", Format(CDate(dateTo), "yyyy-MM-dd"))
                        Adapter1.SelectCommand.Parameters.AddWithValue("@RefAccID", RefData.RefAccID)
                        Adapter1.SelectCommand.Parameters.AddWithValue("@Reference", CInt(reference))

                        Adapter2.SelectCommand.Parameters.AddWithValue("@DateFrom", Format(CDate(dateFrom), "yyyy-MM-dd"))
                        Adapter2.SelectCommand.Parameters.AddWithValue("@DateTo", Format(CDate(dateTo), "yyyy-MM-dd"))
                        Adapter2.SelectCommand.Parameters.AddWithValue("@Reference", CInt(reference))

                        Dim dataSet11 As New DataSet()
                        Adapter1.Fill(dataSet11, "Journal")
                        Adapter2.Fill(dataSet11, "JournalDetails")

                        ' Debug: Log data retrieval
                        System.Diagnostics.Debug.WriteLine($"Retrieved {dataSet11.Tables("Journal").Rows.Count} journal rows")
                        System.Diagnostics.Debug.WriteLine($"Retrieved {dataSet11.Tables("JournalDetails").Rows.Count} detail rows")

                        ' Add beginning balance
                        Dim BegBalce As Decimal = 0
                        Dim R As DataRow = dataSet11.Tables("Journal").NewRow
                        If CheckCalcBegBalance IsNot Nothing AndAlso CheckCalcBegBalance.Checked = True Then
                            BegBalce = GetBegBalanceForRef(CInt(reference), dateFrom, dateTo, currency)
                        End If
                        If BegBalce > 0 Then R("DebitAmount") = BegBalce
                        If BegBalce < 0 Then R("CredAmount") = Math.Abs(BegBalce)
                        R("DocDate") = CDate(dateFrom).AddDays(-1)
                        'R("DocNotes") = "  رصيد مدور"
                        R("DocName") = "  رصيد مدور"
                        R("DocID") = -1
                        R("DocCode") = "X1X"
                        ...
                        ' Create relationship between tables if both tables have data
                        If dataSet11.Tables("Journal").Rows.Count > 0 AndAlso dataSet11.Tables("JournalDetails").Rows.Count > 0 Then
                            Try
                                Dim keyColumn As DataColumn = dataSet11.Tables("Journal").Columns("DocCode")
                                Dim foreignKeyColumn As DataColumn = dataSet11.Tables("JournalDetails").Columns("DocCode")
                                dataSet11.Relations.Add("CategoriesProducts", keyColumn, foreignKeyColumn)
                            Catch relEx As Exception
                                ' Relationship might already exist or columns might not match
                                System.Diagnostics.Debug.WriteLine($"Relationship creation error: {relEx.Message}")
                            End Try
                        End If
                        ...
                        System.Diagnostics.Debug.WriteLine($"Final Journal rows: {dataSet11.Tables("Journal").Rows.Count}")
                        Return dataSet11
                    End Using
                End Using
            End Using
        ...
        Catch ex As Exception
            MessageBox.Show($"خطأ في استرجاع البيانات: {ex.Message}", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error)
            System.Diagnostics.Debug.WriteLine($"Error in GetAccountStatementData: {ex.Message}")
            Return Nothing
        End Try
    End Function
End Class